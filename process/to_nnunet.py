    ...
    def process_entries(entries, images_dest_dir, labels_dest_dir):
        for entry in entries:
            ...
            if dataset_name.lower() == "busi":
                ...
                sitk.WriteImage(image_sitk, dst_image_path)
                ...
                # Remap mask values from 255 to 1
                combined_mask = np.where(combined_mask > 0, 1, 0).astype(np.uint8)
                ...
                sitk.WriteImage(mask_sitk, dst_mask_path)
            else:
                ...
                # Load and convert image (grayscale)
                image = Image.open(src_image_path).convert('L')
                image_np = np.array(image)
                image_sitk = sitk.GetImageFromArray(image_np)
                ...
                # Load and convert mask (grayscale)
                mask = Image.open(src_mask_path).convert('L')
                mask_np = np.array(mask)

                if dataset_name.lower() in ["kvasirseg", "cvc"]:
                    # Remap mask values from 255 to 1
                    mask_np = np.where(mask_np > 0, 1, 0).astype(np.uint8)
                elif dataset_name.lower() == "bkai":
                    # Map mask values: 0 -> 0, 76 -> 1, 150 -> 2
                    value_mapping = {0: 0, 76: 1, 150: 2}
                    mask_np_mapped = np.zeros_like(mask_np, dtype=np.uint8)
                    for orig_val, new_val in value_mapping.items():
                        mask_np_mapped[mask_np == orig_val] = new_val
                    mask_np = mask_np_mapped
                else:
                    # If other datasets, leave mask as is
                    pass
